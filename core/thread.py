...
class Thread:
    ...
    def __init__(
        ...
        if isinstance(recipient, int):
            ...
            if recipient.bot:
                raise CommandError("El receptor no puede ser un BOT.")
    ...
    async def setup(self, *, creator=None, category=None):
        ...
        try:
            channel = await self.bot.modmail_guild.create_text_channel(
                ...
                reason="Creando un canal de hilo...",
            )
        except discord.HTTPException as e:  # Failed to create due to missing perms.
            logger.critical("Un error ocurrió al crear el hilo.", exc_info=True)
            self.manager.cache.pop(self.id)

            embed = discord.Embed(color=self.bot.error_color)
            embed.title = "Error mientras se intentaba crear el hilo"
        ...
        try:
            ...
            logger.error("Un error ocurrió al subir los datos a la base de datos.", exc_info=True)
    ...
    def _format_info_embed(self, user, log_url, log_count, color):
        ...
        embed = discord.Embed(
            color=color, description=f"{user.mention} fue creado {days(created)}", timestamp=time
        )
        ...
        if member is not None:
            ...
            embed.description += f", entró {days(joined)}"
        ...
        if log_count is not None:
            ...
            thread = "hilo" if log_count == 1 else "hilos"
            embed.description += f" con **{log_count or 'ningún'}** {thread}."
        else:
            embed.description += "."
        ...
        if member is None or len(mutual_guilds) > 1:
            embed.add_field(
                name="Servidore(s) compartido(s):", value=", ".join(g.name for g in mutual_guilds)
            )
    ...
    async def _close(
        ...
        try:
            ...
            logger.error("El hilo ya está cerrado: %s.", e)
            return
        ...
        if isinstance(log_data, dict):
            ...
            desc = "No se pudo resolver la URl de los registros."
            log_url = None
    ...
    async def _restart_close_timer(self):
        ...
        if len(re.findall(time_marker_regex, close_message)) == 1:
            ...
            logger.warning(
                "El thread_auto_close_response debe contener sólo un '%s' tiempo especificado.",
                time_marker_regex,
            )
    ...
    async def find_linked_messages(
        ...
        if message1 is not None:
            if (
                ...
                raise ValueError("Mensaje del hilo malformado.")
            ...
            if not (
                ...
                raise ValueError("Mensaje del hilo no encotrado.")
        ...
        async for msg in self.recipient.history():
            ...
            try:
                ...
                continue
        raise ValueError("Mensaje de MD no encontrado.")
    ...
    async def edit_message(self, message_id: typing.Optional[int], message: str) -> None:
        try:
            ...
            logger.warning("Fallo al editar mensaje.", exc_info=True)
            raise
    ...
    async def find_linked_message_from_dm(self, message, either_direction=False):
        ...
        async for linked_message in self.channel.history():
            ...
            if int(msg_id) == message.id:
                return linked_message
        raise ValueError("Canal de mensaje del hilo no encotrado.")
    ...
    async def edit_dm_message(self, message: discord.Message, content: str) -> None:
        ...
        embed.add_field(name="**Editado, mensaje anterior:**", value=embed.description)
    ...
    async def reply(self, message: discord.Message, anonymous: bool = False) -> None:
        ...
        if not any(g.get_member(self.id) for g in self.bot.guilds):
            return await message.channel.send(
                embed=discord.Embed(
                    color=self.bot.error_color,
                    description="Tu mensaje no pudo ser enviado porque "
                    "el receptor no comparte el servidor con el BOT.",
                )
            )
            ...
            if self.close_task is not None:
                ...
                tasks.append(
                    self.channel.send(
                        embed=discord.Embed(
                            color=self.bot.error_color,
                            description="El cierre programado fue cancelado.",
                        )
                    )
                )
    ...
    async def send(
        ...
        for url, filename in images:
            if not prioritize_uploads or (is_image_url(url) and not embedded_image and filename):
                ...
                img_embed.url = url
                img_embed.set_footer(text=f"Subida de imagen adicional ({additional_count})")
        ...
        for url, filename in attachments:
            embed.add_field(
                name=f"Archivo subido ({file_upload_count})", value=f"[{filename}]({url})"
            )
            file_upload_count += 1
        ...
        if from_mod or note:
            ...
            if delete_message and destination == self.channel:
                try:
                    ...
                    logger.warning("No se pudo eliminar el mensaje: %s.", e)

        if from_mod and self.bot.config["dm_disabled"] == 2 and destination != self.channel:
            logger.info("Enviando un mensaje a %s cuando los MDs están deshabilitados.", self.recipient)

        try:
            await destination.trigger_typing()
        except discord.NotFound:
            logger.warning("Canal no encontrado.")
            raise
        ...
        *,
        ...
        if recipient is None and channel is not None:
            ...
            if thread is None:
                ...
                if thread is not None:
                    logger.debug("Hilo encontrado con una ID moderada")
        ...
        if thread is not None:
            ...
            channel = discord.utils.get(
                self.bot.modmail_guild.text_channels, topic=f"ID del usuario: {recipient_id}"
            )
            if channel:
        ...
        *,
        ...
        if thread:
            ...
            if thread.channel and self.bot.get_channel(thread.channel.id):
                logger.warning("Encontrado un hilo existente para %s, creación abortada.", recipient)
                return thread
            logger.warning("Encontrado un hilo existente para %s, cerrando hilo previo.", recipient)
            self.bot.loop.create_task(
                thread.close(closer=self.bot.user, silent=True, delete_channel=False)
            )
