...
def days(day: typing.Union[str, int]) -> str:
    ...
    if day == 0:
        return "**hoy**"
    return f"{day} día atrás" if day == 1 else f"{day} días atrás"
...
def create_not_found_embed(word, possibilities, name, n=2, cutoff=0.6) -> discord.Embed:
    ...
    embed = discord.Embed(
        color=discord.Color.red(), description=f"**{name.capitalize()} `{word}` no pudo ser encontrada.**"
    )
    val = get_close_matches(word, possibilities, n=n, cutoff=cutoff)
    if val:
        embed.description += "\nSin embargo, quizás quisiste decir...\n" + "\n".join(val)
    return embed
