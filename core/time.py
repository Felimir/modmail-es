...
class ShortTime:
    ...
    def __init__(self, argument):
        ...
        if match is None or not match.group(0):
            raise BadArgument("Tiempo proveído inválido.")
...
class HumanTime:
    ...
    def __init__(self, argument):
        ...
        if not status.hasDateOrTime:
            raise BadArgument('Tiempo inválido proveído, intenta con "tomorrow" o con "3 days".')
...
class FutureTime(Time):
    def __init__(self, argument):
        ...
        if self._past:
            raise BadArgument("El tiempo fue en el pasado.")
...
class UserFriendlyTimeSync(Converter):
    ...
    def convert(self, ctx, argument):
        ...
        try:
            ...
            if argument.endswith(" de ahora"):
                argument = argument[:-9].strip()
            # handles "in xxx hours"
            if argument.startswith("en "):
            ...
            if begin not in (0, 1) and end != len(argument):
                raise BadArgument(
                    "El tiempo se encuentra en un lugar inadecuado, lo que debe "
                    "estar al final o al principio de su entrada, o yo "
                    "simplemente no entiendo lo que quieres decir. Lo siento."
                )
            ...
            if begin in (0, 1):
                if begin == 1:
                    ...
                    if argument[0] != '"':
                        raise BadArgument("Cotización esperada antes de la entrada de tiempo...")

                    if not (end < len(argument) and argument[end] == '"'):
                        raise BadArgument("Si se cita la hora, debe desbloquearla.")
            ...
            logger.exception("Algo salió mal mientras analizaba el tiempo.")
            raise
...
def human_timedelta(dt, *, source=None):
    ...
    if len(output) == 2:
        return f"{output[0]} y {output[1]}{suffix}"
    return f"{output[0]}, {output[1]} y {output[2]}{suffix}"
