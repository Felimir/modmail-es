...
class Modmail(commands.Cog):
    ...
    async def setup(self, ctx):
        ...
        if ctx.guild != self.bot.modmail_guild:
            return await ctx.send(
                f"Solo puedes configurar el servidor en: {self.bot.modmail_guild}."
            )

        if self.bot.main_category is not None:
            logger.debug("No se puede reconfigurar el servidor, main_category fue encontrada.")
            return await ctx.send(f"{self.bot.modmail_guild} ya está configurado.")

        if self.bot.modmail_guild is None:
            embed = discord.Embed(
                title="Error",
                description="Servidor de funcionamiento del BOT no encontrado.",
        ...
        for level in PermissionLevel:
            ...
            for perm in permissions:
                ...
                if key is not None:
                    logger.info("Garantizando acceso %s a la categoría del BOT.", key.name)
                    overwrites[key] = discord.PermissionOverwrite(read_messages=True)

        category = await self.bot.modmail_guild.create_category(
            name="Hilos", overwrites=overwrites
        )

        await category.edit(position=0)

        log_channel = await self.bot.modmail_guild.create_text_channel(
            name="registros", category=category
        )

        embed = discord.Embed(
            title="Recordatorio",
            description=f"Puedes usar el comando `{self.bot.prefix}config set log_channel_id "
            "<channel-id>` para establecer el canal de registros, y luego puedes eliminar este, establecido por defecto. "
            f"{log_channel.mention} log channel.",
            color=self.bot.main_color,
        )

        embed.set_footer(text=f'Ejecuta "{self.bot.prefix}help" para una lista completa de los comandos.')
        ...
        await ctx.send(
            ...
            "Recuerda establecer niveles de permisos para dar acceso a roles "
            "o usuarios la habilidad de usar al BOT.\n\n"
            f"Ejecuta:\n- `{self.bot.prefix}permissions` y `{self.bot.prefix}permissions add` "
            "para más información o .\n"
            f"- `{self.bot.prefix}config help` para una lista de las personalizaciones disponibles."
        )
    ...
    async def snippet(self, ctx, *, name: str.lower = None):
        ...
        if name is not None:
            ...
            if val is None:
                embed = create_not_found_embed(name, self.bot.snippets.keys(), "Respuestas predefinidas")
            else:
                embed = discord.Embed(
                    title=f'Respuestas predefinidas - "{name}":', description=val, color=self.bot.main_color
                )
            return await ctx.send(embed=embed)

        if not self.bot.snippets:
            embed = discord.Embed(
                color=self.bot.error_color, description="No hay respuestas predefinidas establecidas."
            )
            embed.set_footer(text=f'Chequea "{self.bot.prefix}help snippet add" para añadir una respuesta predefinida.')
            embed.set_author(name="Respuestas predefinidas", icon_url=ctx.guild.icon_url)
            return await ctx.send(embed=embed)
    ...
    async def snippet_raw(self, ctx, *, name: str.lower):
        ...
        if val is None:
            ...
            embed = discord.Embed(
                title=f'Respuesta predefinida (RAW) - "{name}":',
                description=f"```\n{val}```",
                color=self.bot.main_color,
            )
    ...
    async def snippet_add(self, ctx, name: str.lower, *, value: commands.clean_content):
        ...
        if name in self.bot.snippets:
            embed = discord.Embed(
                title="Error",
                color=self.bot.error_color,
                description=f"La respuesta predefinida `{name}` ya existe.",
            )
            return await ctx.send(embed=embed)
        ...
        if len(name) > 120:
            embed = discord.Embed(
                title="Error",
                color=self.bot.error_color,
                description="El nombre de la respuesta predefinida no puede superar los 120 caracteres.",
            )
            return await ctx.send(embed=embed)
        ...
        embed = discord.Embed(
            title="Respuesa predefinida añadida",
            color=self.bot.main_color,
            description="Respuesta predefinida añadida exitosamente.",
        )
        return await ctx.send(embed=embed)
    ...
    async def snippet_remove(self, ctx, *, name: str.lower):
        ...
        if name in self.bot.snippets:
            embed = discord.Embed(
                title="Respuesta predefinida removida",
                color=self.bot.main_color,
                description=f"La respuesta predefinda `{name}` ahora está eliminada.",
            )
            self.bot.snippets.pop(name)
            await self.bot.config.update()
        else:
            embed = create_not_found_embed(name, self.bot.snippets.keys(), "Respuesta predefinida")
    ...
    async def snippet_edit(self, ctx, name: str.lower, *, value):
        ...
        if name in self.bot.snippets:
            ...
            embed = discord.Embed(
                title="Respuesta predefinida editada",
                color=self.bot.main_color,
    ...
    async def send_scheduled_close_message(self, ctx, after, silent=False):
        ...
        silent = "**silencionsamente** " if silent else ""

        embed = discord.Embed(
            title="Cierre programado",
            description=f"Este hilo se cerrará {silent}en {human_delta}.",
            color=self.bot.error_color,
        )
        ...
        embed.set_footer(text="El cierre será cancelado si el hilo recibe un mensaje.")
    ...
    async def close(self, ctx, *, after: UserFriendlyTime = None):
        ...
        if cancel:
            ...
            if thread.close_task is not None or thread.auto_close_task is not None:
                ...
                embed = discord.Embed(
                    color=self.bot.error_color,
                    description="Este hilo aún no ha sido programado para cerrarse.",
                )
    ...
    async def notify(
        ...
        if mention in mentions:
            embed = discord.Embed(
                color=self.bot.error_color,
                description=f"{mention} ya va a ser notificado.",
            )
        else:
            ...
            embed = discord.Embed(
                color=self.bot.main_color,
                description=f"{mention} será notificado en el próximo mensaje.",
            )
        return await ctx.send(embed=embed)
    ...
    async def unnotify(
        ...
        if mention not in mentions:
            embed = discord.Embed(
                color=self.bot.error_color,
                description=f"{mention} no tiene notificaciones pendientes.",
            )
        else:
    ...
    async def subscribe(
        ...
        mention = self.parse_user_or_role(ctx, user_or_role)
        if mention is None:
            raise commands.BadArgument(f"{user_or_role} no es un usuario o un rol válido.")
        ...
        if mention in mentions:
            embed = discord.Embed(
                color=self.bot.error_color,
                description=f"{mention} ya está subscripto al hilo.",
            )
        else:
            mentions.append(mention)
            await self.bot.config.update()
            embed = discord.Embed(
                color=self.bot.main_color,
                description=f"{mention} ahora será notificado de todos los mensajes que lleguen.",
            )
        return await ctx.send(embed=embed)
    ...
    async def unsubscribe(
        ...
        if mention not in mentions:
            embed = discord.Embed(
                color=self.bot.error_color,
                description=f"{mention} no está subscripto a este hilo.",
            )
        else:
            mentions.remove(mention)
            await self.bot.config.update()
            embed = discord.Embed(
                color=self.bot.main_color,
                description=f"{mention} ahora está dessubscripto a este hilo.",
            )
        return await ctx.send(embed=embed)
    ...
    def format_log_embeds(self, logs, avatar_url):
        ...
        for entry in logs:
            ...
            if closer is not None:
                ...
                logger.debug("Entrada de registros inválida: no hay un cerrante.")
                embed.add_field(name="Clave de registros:", value=f"`{entry['key']}`")
    ...
    async def logs_closed_by(self, ctx, *, user: User = None):
        ...
        if not embeds:
            embed = discord.Embed(
                color=self.bot.error_color,
                description="No hay entradas en los registros que coinicidan con esta búsqueda.",
            )
            return await ctx.send(embed=embed)
    ...
    async def logs_delete(self, ctx, key_or_link: str):
        ...
        if not success:
            embed = discord.Embed(
                title="Error",
                description=f"Entrada de registros `{key}` no encontrada.",
                color=self.bot.error_color,
            )
        else:
            embed = discord.Embed(
                title="Correcto",
                description=f"Entrada de registros`{key}` correctamente eliminada.",
                color=self.bot.main_color,
            )
    ...
    async def logs_responded(self, ctx, *, user: User = None):
        ...
        if not embeds:
            embed = discord.Embed(
                color=self.bot.error_color,
                description=f"{getattr(user, 'mention', user.id)} no ha respondido a ningún hilo.",
            )
            return await ctx.send(embed=embed)
    ...
    async def logs_search(self, ctx, limit: Optional[int] = None, *, query):
        ...
        if not embeds:
            embed = discord.Embed(
                color=self.bot.error_color,
                description="No hay entradas en los registros que coincidan con esa búsqueda.",
            )
            return await ctx.send(embed=embed)
    ...
    async def edit(self, ctx, message_id: Optional[int] = None, *, message: str):
        ...
        try:
            ...
            return await ctx.send(
                embed=discord.Embed(
                    title="Fallo",
                    description="No se pudo encontrar un mensaje para editar.",
                    color=self.bot.error_color,
                )
            )
    ...
    async def contact(
        ...
        *,
        ...
        if user.bot:
            embed = discord.Embed(
                color=self.bot.error_color, description="No se puede iniciar un hilo con un BOT."
            )
            return await ctx.send(embed=embed)
        ...
        if exists:
            embed = discord.Embed(
                color=self.bot.error_color,
                description="Un hilo con este usuario "
            ...
            if self.bot.config["dm_disabled"] >= 1:
                logger.info("Contactando con el usuario %s cuando los MDs están desactivados.", user)

            embed = discord.Embed(
                title="Hilo creado",
                description=f"Hilo iniciado por {ctx.author.mention} para {user.mention}.",
                color=self.bot.main_color,
            )
    ...
    async def blocked(self, ctx):
        ...
        if users:
            ...
            for mention, reason in users:
                line = mention + f" - {reason or 'No se ha dado una razón'}\n"
            ...
            embeds[0].description = "No hay ningún usuario bloqueado."
    ...
    async def block(self, ctx, user: Optional[User] = None, *, after: UserFriendlyTime = None):
        ...
        reason = f"Por {escape_markdown(ctx.author.name)}#{ctx.author.discriminator}"

        if after is not None:
            if "%" in reason:
                raise commands.BadArgument('La razón contiene un caracter ilegal ("%").')
            if after.arg:
                reason += f" por `{after.arg}`"
            if after.dt > after.now:
                reason += f" hasta {after.dt.isoformat()}"
        ...
        if str(user.id) in self.bot.blocked_users and msg:
            ...
            embed = discord.Embed(
                title="Correcto",
                description=f"{mention} que fue previamente bloqueado por {old_reason}.\n"
            ...
            embed = discord.Embed(
                title="Correcto",
                color=self.bot.main_color,
                description=f"{mention} ahoar está bloqueado por {reason}",
            )
    ...
    async def unblock(self, ctx, *, user: User = None):
        ...
        if str(user.id) in self.bot.blocked_users:
            ...
            if msg.startswith("System Message: "):
                ...
                embed = discord.Embed(
                    title="Correcto",
                    description=f"{mention} que fue previamente bloqueado internamente por {reason}.\n"
                    f"{mention} ahora no está bloqueado.",
                    color=self.bot.main_color,
                )
                embed.set_footer(
                    text="Sin embargo, si la razón del bloqueo del sistema original sigue siendo aplicable, "
                    f"{name} será automáticamente bloqueado nuevamente. "
                    f'Usa "{self.bot.prefix}blocked whitelist {user.id}" para poner al usuario en la lista blanca.'
                )
            else:
                embed = discord.Embed(
                    title="Correcto",
                    color=self.bot.main_color,
                    description=f"{mention} ahora ya no está bloqueado.",
                )
        else:
            embed = discord.Embed(
    ...
    async def delete(self, ctx, message_id: int = None):
        ...
        try:
            ...
            logger.warning("Fallo al eliminar el mensaje: %s.", e)
            return await ctx.send(
                embed=discord.Embed(
                    title="Fallo",
                    description="No se pudo buscar un mensaje para eliminar.",
                    color=self.bot.error_color,
                )
            )
    ...
    async def repair(self, ctx):
        ...
        if ctx.thread:
            ...
            if user_id == -1:
                logger.info("Estableciendo la descripción del canal a la ID del usuario...")
                await ctx.channel.edit(topic=f"ID del usuario: {ctx.thread.id}")
            return await self.bot.add_reaction(ctx.message, sent_emoji)

        logger.info("Intentando arreglar un hilo roto %s.", ctx.channel.name)
        ...
        if thread is not None:
            logger.debug("Encontrado un hilo con la ID moderada.")
            await ctx.channel.edit(reason="Arreglando hilo roto", topic=f"ID del usuario: {user_id}")
            return await self.bot.add_reaction(ctx.message, sent_emoji)
        ...
        async for message in ctx.channel.history(limit=10, oldest_first=True):
            if (
                ...
                if user_id != -1:
                    ...
                    thread.ready = True
                    logger.info(
                        "Estableciendo la descripciión del canal actual a la ID del usuario y creando nuevo hilo..."
                    )
                    await ctx.channel.edit(
                        reason="Arreglando hilo roto", topic=f"ID del usuario: {user_id}"
                    )
                    return await self.bot.add_reaction(ctx.message, sent_emoji)

        else:
            logger.warning("No se encontró ningún mensaje de génesis.")
        ...
        if m is not None:
            ...
            if len(users) == 1:
                ...
                if user.id in self.bot.threads.cache:
                    ...
                    if thread.channel:
                        embed = discord.Embed(
                            ...
                            description="Este hilo ya no está en uso. "
                            f"Todos los mensajes serán redireccionados a {ctx.channel.mention} en su lugar.",
                            color=self.bot.error_color,
                        )
                        embed.set_footer(
                            text='Por favor elimina este canal manualmente, no utilices "{prefix}close".'
                        )
                        try:
                            await thread.channel.send(embed=embed)
                ...
                thread.ready = True
                logger.info("Estableciendo la descripción del canal a la ID del usuario y creando nuevo hilo...")
                await ctx.channel.edit(
                    reason="Arreglando hilo roto", name=name, topic=f"ID del usuario: {user.id}"
                )
                return await self.bot.add_reaction(ctx.message, sent_emoji)

            elif len(users) >= 2:
                logger.info(" usuarios con el mismo nombre y discriminador.")
    ...
    async def isenable(self, ctx):
        ...
        if self.bot.config["dm_disabled"] == 1:
            embed = discord.Embed(
                title="Nuevos hilos deshabilitados",
                description="No se están recibiendo nuevos hilos, se están respondiendo los actuales.",
                color=self.bot.error_color,
            )
        elif self.bot.config["dm_disabled"] == 2:
            embed = discord.Embed(
                title="Todos los hilos deshabilitados",
                description="No se están recibiendo nuevos hilos, tampoco se están respondiendo los actuales..",
                color=self.bot.error_color,
            )
        else:
            embed = discord.Embed(
                title="Activado",
                description="Se están recibiendo y respondiendo todos los hilos.",
                color=self.bot.main_color,
            )
