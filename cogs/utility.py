...
class ModmailHelpCommand(commands.HelpCommand):
    async def format_cog_help(self, cog, *, no_cog=False):
        ...
        for format_ in formats:
            description = (
                cog.description or "Sin descripcióm."
                if not no_cog
                else "Comandos varios sin categoría."
            )
    ...
    async def send_error_message(self, error):
        ...
        if val is not None:
            embed = discord.Embed(
                title=f"{command} es una respuesta predefinida", color=self.context.bot.main_color
            )
        ...
        if val is not None:
            ...
            if not values:
                embed = discord.Embed(
                    ...
                    "Este alias ahora será eliminado",
                )
                ...
                if len(values) == 1:
                    ...
                    embed.add_field(name=f"`{command}` señala a:", value=values[0])
                else:
                    embed = discord.Embed(
                        title=f"{command} es un alias.",
                        color=self.context.bot.main_color,
                        description=f"**`{command}` señala los siguientes pasos:**",
                    )
            ...
            embed.set_footer(
                ...
                "para más detalles en los alias."
            )
            return await self.get_destination().send(embed=embed)
        ...
        embed.set_footer(text=f'Comando o categoría "{command}" no encontrada.')
        ...
        if closest:
            embed.add_field(name="Quizá quisiste decir:", value="\n".join(f"`{x}`" for x in closest))
        else:
            embed.title = "No se pudo buscar el comando o la categoría
            embed.set_footer(
...
class Utility(commands.Cog):
    ...
    async def changelog(self, ctx, version: str.lower = ""):
        ...
        try:
            ...
            try:
                ...
                logger.warning("Fallo al mostrar el registro de cambios.", exc_info=True)
                await ctx.send(
                    f"Encuentra el registro de cambios aquí: {changelog.latest_version.changelog_url}#v{version[::2]}"
                )
    ...
    async def about(self, ctx):
        ...
        embed.set_author(
            name="BOT - Información",
            icon_url=self.bot.user.avatar_url,
            url="https://discordapp.com",
        )
        embed.set_thumbnail(url=self.bot.user.avatar_url)

        desc = "Un BOT de Discord basado en 'ModMail', pero privado para el servidor. "
        desc += "Permite comunicarse con el Staff de una manera más organizada"
        embed.description = desc

        embed.add_field(name="Tiempo de ejecución", value=self.bot.uptime)
        embed.add_field(name="Latencia", value=f"{self.bot.latency * 1000:.2f} ms")
        embed.add_field(name="Versión", value=f"`{self.bot.version}`")
        embed.add_field(name="Desarrollador & Traductor:", value="`Felimir`")
        ...
        if self.bot.version.is_prerelease:
            ...
            footer = (
                f"Estás en la preversión • la última versión es v{stable.version}."
            )
        elif self.bot.version < parse_version(latest.version):
            footer = f"Una nueva versión está disponible v{latest.version}."
        else:
            footer = "Estás en la última versión lanzada. Felicidades!"
    ...
    async def debug(self, ctx):
        ...
        if not logs:
            embed = discord.Embed(
                ...
                description="No hay registros por el momento.",
            )
            embed.set_footer(text="Ve a la Consola del host para ver más registros")
        ...
        embed.set_footer(text="Registros de Depuración, navega con las reacciones")
    ...
    async def debug_hastebin(self, ctx):
        ...
        try:
            ...
            embed = discord.Embed(
                title="Registros de Depuración",
                color=self.bot.main_color,
                description="Algo está mal. No puedo subir los registros a Hastebin.",
            )
            embed.set_footer(text="Ve a la Consola del host para ver más registros.")
        await ctx.send(embed=embed)
    ...
    async def debug_clear(self, ctx):
        ...
        await ctx.send(
            embed=discord.Embed(
                color=self.bot.main_color, description="Los registros en caché fueron limpiados"
            )
        )
    ...
    async def activity(self, ctx, activity_type: str.lower, *, message: str = ""):
        ...
        if activity_type == "clear":
            ...
            embed = discord.Embed(title="Actividad removida", color=self.bot.main_color)
            return await ctx.send(embed=embed)
    ...
    async def status(self, ctx, *, status_type: str.lower):
        ...
        if status_type == "clear":
            ...
            await self.bot.config.update()
            await self.set_presence()
            embed = discord.Embed(title="Estado removido", color=self.bot.main_color)
            return await ctx.send(embed=embed)
    ...
    async def set_presence(self, *, status=None, activity_type=None, activity_message=None):
        ...
        if activity_type is not None and not activity_message:
            logger.warning(
                'No se encuentra ningún mensaje de actividad mientras se proporciona la actividad, por defecto a "Modmail".'
            )
            activity_message = "Envíame un mensaje con tu duda, problema o reporte!"

        if activity_type == ActivityType.listening:
            if activity_message.lower().startswith("a "):
    ...
    async def loop_presence(self):
        ...
        logger.debug("Reiniciando la presencia...")
        await self.set_presence()
    ...
    async def before_loop_presence(self):
        ...
        if activity is not None:
            msg = f"Presencia establecida a: {activity.type.name.capitalize()} "
            if activity.type == ActivityType.listening:
                msg += f"a {activity.name}."
            else:
                msg += f"{activity.name}."
            ...
            logger.info("No se estableció ninguna presencia.")
        if status is not None:
            msg = f"Presencia establecida a: {status.value}."
            logger.info(msg)
        else:
            logger.info("No se estableció ninguna presencia.")

        await asyncio.sleep(1800)
        logger.info("Iniciando presencia en bucle...")
    ...
    async def mention(self, ctx, *, mention: str = None):
        ...
        if mention is None:
            ...
            embed = discord.Embed(
                ...
                description=f'Cuando se cree un hilo, se mencionará a "{mention}".',
                color=self.bot.main_color,
            )
    ...
    async def prefix(self, ctx, *, prefix=None):
        ...
        embed = discord.Embed(
            title="Prefijo actual", color=self.bot.main_color, description=f"{current}"
        )
        ...
        if key in keys:
            try:
                ...
                embed = discord.Embed(
                    title="Correcto",
                    color=self.bot.main_color,
                    description=f"Se estableció `{key}` a `{self.bot.config[key]}`.",
                )
    ...
    async def config_remove(self, ctx, *, key: str.lower):
        ...
        if key in keys:
            ...
            embed = discord.Embed(
                title="Success",
                color=self.bot.main_color,
                description=f"`{key}` fue reiniciada a su valor por defecto.",
            )
        else:
            embed = discord.Embed(
                title="Error", color=self.bot.error_color, description=f"{key} es una clave inválida."
    ...
    async def config_get(self, ctx, *, key: str.lower = None):
        ...
        if key:
            if key in keys:
                ...
                embed.set_author(name="Claves de configuración", icon_url=self.bot.user.avatar_url)
                ...
                embed.set_footer(
                    text=f'Ejecuta "{self.bot.prefix}config options" para una lista de las claves de configuración válidas.'
                )
    ...
    async def config_help(self, ctx, key: str.lower = None):
        ...
        if key is not None and not (
            ...
            if closest:
                embed.add_field(
                    name=f"Quizá quisiste decir:", value="\n".join(f"`{x}`" for x in closest)
                )
            return await ctx.send(embed=embed)
        ...
        if key is not None and key not in config_help:
            embed = discord.Embed(
                title="Error",
                color=self.bot.error_color,
                description=f"No se encontraron resultados de ayuda para `{key}`.",
            )
            return await ctx.send(embed=embed)
        ...
        for i, (current_key, info) in enumerate(config_help.items()):
            if current_key == key:
                index = i
            embed = discord.Embed(
                title=f"Descripción de configuración para {current_key}:", color=self.bot.main_color
            )
    ...
    async def alias(self, ctx, *, name: str.lower = None):
        ...
        if not self.bot.aliases:
            embed = discord.Embed(
                color=self.bot.error_color, description="No hay alias por el momento."
            )
    ...
    async def alias_raw(self, ctx, *, name: str.lower):
        ...
        embed = discord.Embed(
            title=f'Alias (RAW) - "{name}":', description=f"```\n{val}```", color=self.bot.main_color
        )
    ...
    async def make_alias(self, name, value, action):
        ...
        if not values:
            embed = discord.Embed(
                title="Error",
                color=self.bot.error_color,
                description="Alias de múltiples pasos inválidos, intenta envolver cada paso entre comillas.",
            )
        ...
        if len(values) > 25:
            embed = discord.Embed(
                title="Error", description="Demasiados pasos, un máximo de 25.", color=self.bot.error_color
            )
            return embed
        ...
        if not multiple_alias:
            embed.add_field(name=f"`{name}` señala a:", value=utils.truncate(values[0], 1024))
        else:
            embed.description = f"`{name}` ahora señala a los siguientes pasos:"
            ...
            if not self.bot.get_command(linked_command):
                ...
                if alias_command is not None:
                    ...
                    if multiple_alias:
                        ...
                        embed.description = (
                            "El comando que intentas señalar "
    ...
    async def alias_add(self, ctx, name: str.lower, *, value):
        ...
        if self.bot.get_command(name):
            embed = discord.Embed(
                title="Error",
                color=self.bot.error_color,
                description=f"Un comando con el mismo alias existe: `{name}`.",
            )
    ...
    async def alias_remove(self, ctx, *, name: str.lower):
        ...
        if name in self.bot.aliases:
            ...
            embed = discord.Embed(
                ...
                color=self.bot.main_color,
                description=f"Removido correctamente el alias `{name}`.",
            )
        else:
            embed = utils.create_not_found_embed(name, self.bot.aliases.keys(), "Alias")
    ...
    def _verify_user_or_role(user_or_role):
        ...
        raise commands.BadArgument(f'El usuario o el rol "{user_or_role}" no fue encontrado!')
    ...
    async def permissions_override(self, ctx, command_name: str.lower, *, level_name: str):
        ...
        if level is PermissionLevel.INVALID:
            ...
            logger.info(
                "Permisos de nivel actualizados para un comando, de `%s` a `%s`.",
            ...
            embed = discord.Embed(
                title="Correcto",
                color=self.bot.main_color,
                description="Niveles de permisos para el comando "
                f"`{command.qualified_name}` establecidos a `{level.name}`.",
            )
        return await ctx.send(embed=embed)
        ...
        *,
        ...
        if type_ == "command":
            ...
            if level > PermissionLevel.REGULAR:
                ...
                if key is not None:
                    logger.info("Dando acceso a %s a la categoría de hilos...", key.name)
                    await self.bot.main_category.set_permissions(key, read_messages=True)

        embed = discord.Embed(
            title="Correcto",
            color=self.bot.main_color,
            description=f"Los permisos para `{name}` fueron actualizados.",
        )
        return await ctx.send(embed=embed)
    ...
    async def permissions_remove(
        ...
        *,
        ...
        if type_ == "override":
            ...
            if level is None:
                ...
                embed = discord.Embed(
                    title="Error",
                    color=self.bot.error_color,
                    description=f"El nivel de permiso de comando nunca fue anulado: `{name}`, "
                    f"el permiso actual es {perm.name}.",
                )
            else:
                logger.info("Restablecido el nivel de permiso de comando para `%s`.", name)
                ...
                embed = discord.Embed(
                    title="Correcto",
                    color=self.bot.main_color,
                    description=f"Nivel de permiso de comando para `{name}` fue correctamente restablecido a {perm.name}.",
                )
            return await ctx.send(embed=embed)
        ...
        if type_ == "level":
            if level > PermissionLevel.REGULAR:
                if value == -1:
                    logger.info("Denegando acceso a @everyone...")
                    await self.bot.main_category.set_permissions(
                        self.bot.modmail_guild.default_role, read_messages=False
                    )
                elif isinstance(user_or_role, discord.Role):
                    logger.info("Denegando acceso a %s para la categoría de hilos.", user_or_role.name)
                    ...
                    if member is not None and member != self.bot.modmail_guild.me:
                        logger.info("Denegando acceso a %s para la categoría de hilos.", member.name)
            ...
            if user_or_role == "override":
                if name is None:
                    ...
                    if not overrides:
                        embeds.append(
                            discord.Embed(
                                title="Permisos anulados",
                                description="No tienes permisos de niveles de comandos anulados por el momento.",
                                color=self.bot.error_color,
                            )
                        )
                    else:
                        for items in zip_longest(*(iter(sorted(overrides.items())),) * 15):
                            ...
                            embed.set_author(
                                name="Permisos anulados", icon_url=ctx.guild.icon_url
                            )
                            embeds.append(embed)
                ...
                if level is None:
                    embed = discord.Embed(
                        title="Error",
                        color=self.bot.error_color,
                        description=f"El nivel de permisos del comando nunca fue anulado: `{name}`, "
                    ...
                    embed = discord.Embed(
                        title="Correcto",
                        color=self.bot.main_color,
                        description=f'Anulación de permisos para el comando "{name}" es "{perm.name}".',
                    )
    ...
    async def oauth_whitelist(self, ctx, target: Union[discord.Role, utils.User]):
        ...
        embed.description = (
            f"{'Quitado a ' if removed else 'Agregado a '}{target.mention}  a la lista blanca para ver los registros."
        )
    ...
    async def oauth_show(self, ctx):
        ...
        embed = discord.Embed(color=self.bot.main_color)
        embed.title = "Lista blanca Oauth"
    ...
    async def eval_(self, ctx, *, body: str):
        ...
        logger.warning("Corriendo comando eval:\n%s", body)
