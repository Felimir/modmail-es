...
class ModmailBot(commands.Bot):
    def __init__(self):
        ...
        if mongo_uri is None:
            logger.critical("Una URI de Mongo es necesaria.")
            raise RuntimeError
    ...
    def startup(self):
        ...
        for cog in self.loaded_cogs:
            logger.debug("Cargando %s...", cog)
            try:
                self.load_extension(cog)
                logger.debug("Correctamente cargado %s.", cog)
            except Exception:
                logger.exception("Fallo al cargar %s.", cog)
        logger.line("debug")
            ...
            if self.get_channel(int(log["channel_id"])) is None:
                ...
                log_data = await self.api.post_log(
                    log["channel_id"],
                    {
                        ...
                        "close_message": "El canal fue eliminado, no se encontró un cerrante.",
                        "closer": {
                            "id": str(self.user.id),
                            "name": self.user.name,
        ...
        if thread is None:
            ...
            if delta:
                await message.channel.send(
                    embed=discord.Embed(
                        title="Mensaje no enviado!",
                        description=f"Debes esperar por {delta} para enviarme un mensaje nuevamente.",
                        color=self.error_color,
                    )
                )
                return
    ...
    async def on_message_delete(self, message):
        ...
        if isinstance(message.channel, discord.DMChannel):
            ...
            try:
                ...
                if str(e) != "Mensaje del canal del hilo no encontrado.":
    ...
    async def on_command_error(self, context, exception):
        if isinstance(exception, commands.BadUnionArgument):
            msg = "No se pudo buscar el especificado " + human_join(
